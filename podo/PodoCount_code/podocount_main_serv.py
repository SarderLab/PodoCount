...
for WSI in WSIs:
    ...
    for bb in range(len(bbs)):
        ...
        if args.type == InputType.Human_Analysis.value:
            xml_counter, xml_contour, count, pod_feat_vector = get_pod_props(roi,glom_mask,slider,x_start,y_start,xml_counter,xml_contour,count,dist_mpp,area_mpp2, ihc_gauss_sd, dt_gauss_sd, max_major, min_minor, max_ecc, emt_thresh)
        elif args.type == InputType.Mouse_Analysis.value:
            xml_counter, xml_contour, count, pod_feat_vector = get_pod_props_mouse(roi,glom_mask,slider,x_start,y_start,xml_counter,xml_contour,count,dist_mpp,area_mpp2, ihc_gauss_sd, dt_gauss_sd, max_major, min_minor, max_ecc, emt_thresh)
